...
# Keep Decky's FEX-hosted backend responsive under game load: Nice/CPUWeight
# lift PluginLoader above the game's SCHED_OTHER threads without SCHED_RR's
# realtime-budget edge cases (RT throttling shared with gamescope RR).
install -d -m 0755 /etc/systemd/system/plugin_loader.service.d
cat > /etc/systemd/system/plugin_loader.service.d/10-decky-priority.conf <<'EOF'
[Service]
Nice=-6
CPUWeight=400
EOF
